...
def update_init_py(project_root: Path, parsed_nodes: List[Dict[str, Any]], dry_run: bool):
  ...
  for node in parsed_nodes:
    # node['type'] is the Python class name (e.g., "APGGuiderNode")
    class_name_str = node['type'] 
    node_title = node['title']
        
        # CRITICAL FIX: node['key'] is the original, backwards-compatible mapping key 
        # (e.g., "APGGuiderForked")
    ...
    # Import ClassName from relative path
    imports_set.add(f"from .{relative_import_path} import {class_name_str}")

    # MAPPING: Key MUST be the original key, Value MUST be the class name
    mapping_entries.append(f'  "{mapping_key}": {class_name_str}')

    display_map[mapping_key] = node_title

  imports = "\n".join(sorted(list(imports_set)))
  mappings_str_literal = "{\n" + ",\n".join(sorted(mapping_entries)) + "\n}"
  display_str = json.dumps(display_map, indent=4, sort_keys=True)
...
NODE_CLASS_MAPPINGS = {mappings_str_literal}

NODE_DISPLAY_NAME_MAPPINGS = {display_str}

# WEB_DIRECTORY = "./web"

__all__ = ['NODE_CLASS_MAPPINGS', 'NODE_DISPLAY_NAME_MAPPINGS']

print(f"[ComfyUI_MD_Nodes] Initialized ({len(parsed_nodes)} nodes)")
'''
  if dry_run: print(f" (Dry run: would write {len(parsed_nodes)} nodes to __init__.py)"); return
  try:
    with open(file_path, 'w', encoding='utf-8') as f: f.write(content)
    print(f" ✓ __init__.py updated with {len(parsed_nodes)} nodes.")
  except Exception as e:
